...
import type { DiscoveryTimelineEvent } from '@/types/prospect-discovery';

type Props = {
  events: DiscoveryTimelineEvent[];
};

export function DiscoveryTimeline({ events }: Props) {
  return (
    <div className="rounded-xl border border-gold-500/12 bg-maroon-900 p-5">
      <h3 className="text-sm font-semibold text-ink-500 mb-4">Discovery Timeline</h3>
          ...
          <div key={event.id} className="flex gap-3">
            <div className="flex flex-col items-center">
              <div className={cn(
                ...
                event.completed ? 'border-brand-500 bg-gradient-to-r from-gold-400 to-gold-300/10 text-brand-400' : 'border-gold-500/12 bg-card-900 text-ink-500'
              )}>
            ...
            <div className="pb-4 flex-1">
              <p className={cn('text-sm font-medium', event.completed ? 'text-ink-500' : 'text-ink-500')}>{event.label}</p>
