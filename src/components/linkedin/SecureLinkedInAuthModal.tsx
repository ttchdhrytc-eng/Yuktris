...
export function SecureLinkedInAuthModal({ open, loginUrl, identityVerified, securityCheckRequired, repeatedSecurityChecks = false, queueItemId, recovering = false, connectionFailed = false, onRecover, onCancel }: Props) {
  ...
  }, [open, identityVerified, queueItemId]);
      ...
      <div className="flex h-[min(900px,94vh)] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-gold-500/20 bg-maroon-900 shadow-2xl">
        <header className="flex shrink-0 items-center justify-between border-b border-gold-500/15 px-4 py-3 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="text-sm font-semibold text-ink-50">{securityCheckRequired ? 'LinkedIn verification required' : 'Sign in to LinkedIn once'}</h2><p className="text-xs text-ink-400">{securityCheckRequired ? "Complete LinkedIn's security check directly in this secure view. Yuktris never collects verification codes." : 'Enter your credentials directly on linkedin.com. Yuktris remains passive and never sees your password.'}</p></div>
          </div>
        ...
        <div className="relative min-h-0 flex-1 bg-maroon-950">
          ...
          {(!safeUrl || covered || recovering || connectionFailed) && <div className="absolute inset-0 z-10 flex items-center justify-center bg-maroon-950 px-6 text-center"><div className="max-w-md space-y-3">
            {covered ? <ShieldCheck className="mx-auto h-10 w-10 text-success-500" /> : connectionFailed ? <AlertTriangle className="mx-auto h-10 w-10 text-warning-400" /> : <Loader2 className="mx-auto h-9 w-9 animate-spin text-brand-300" />}
            <p className="text-base font-semibold text-ink-50">{covered ? 'Securing your LinkedIn connection...' : connectionFailed ? "We couldn't open the secure LinkedIn window." : recovering ? 'Reconnecting secure LinkedIn session...' : 'Opening secure LinkedIn session...'}</p>
            <p className="text-sm text-ink-400">{covered ? 'Authentication succeeded. Yuktris is persisting this browser Context for future sessions.' : connectionFailed ? 'The backend authentication remains active while Yuktris recovers this same session.' : securityCheckRequired ? 'Complete LinkedIn verification directly in the secure browser.' : 'Sign in directly on linkedin.com. Yuktris does not inspect or store your password or verification codes.'}</p>
            {connectionFailed && onRecover && <Button onClick={onRecover} disabled={recovering}><RefreshCw className="h-4 w-4" /> Reconnect secure window</Button>}
          </div></div>}
        </div>
