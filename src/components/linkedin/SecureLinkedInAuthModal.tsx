      ...
      <div className="flex h-[min(900px,94vh)] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-gold-500/20 bg-maroon-900 shadow-2xl">
        <header className="flex shrink-0 items-center justify-between border-b border-gold-500/15 px-4 py-3 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="text-sm font-semibold text-ink-50">LinkedIn verification required</h2><p className="text-xs text-ink-400">Complete LinkedIn's security check directly in this secure view. Yuktris never collects verification codes.</p></div>
          </div>
        ...
        <div className="relative min-h-0 flex-1 bg-maroon-950">
          ...
          {(!safeUrl || covered || recovering || connectionFailed) && <div className="absolute inset-0 z-10 flex items-center justify-center bg-maroon-950 px-6 text-center"><div className="max-w-md space-y-3">
            ...
            <p className="text-base font-semibold text-ink-50">{covered ? 'Securing your LinkedIn connection...' : connectionFailed ? "We couldn't open the secure LinkedIn verification window." : recovering ? 'Reconnecting secure LinkedIn verification...' : 'Preparing secure LinkedIn verification...'}</p>
            <p className="text-sm text-ink-400">{covered ? 'You can safely remain in Yuktris while encrypted session setup completes.' : connectionFailed ? 'The backend authentication remains active while Yuktris recovers this same verification session.' : 'LinkedIn requested human verification. The secure view will appear here without interrupting the backend queue.'}</p>
