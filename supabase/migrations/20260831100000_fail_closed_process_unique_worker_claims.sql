...
CREATE OR REPLACE FUNCTION public.is_process_unique_worker_identity(p_worker_id text)
RETURNS boolean LANGUAGE sql IMMUTABLE PARALLEL SAFE SET search_path=pg_catalog,public AS $$
  SELECT coalesce(p_worker_id ~ '^v1:[A-Za-z0-9][A-Za-z0-9._-]{0,127}:[A-Za-z0-9][A-Za-z0-9._-]{0,127}:[A-Za-z0-9][A-Za-z0-9._-]{0,127}:[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89aAbB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$',false)
$$;
...
REVOKE ALL ON FUNCTION public.is_process_unique_worker_identity(text),public.claim_queue_task(text,integer) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.is_process_unique_worker_identity(text),public.claim_queue_task(text,integer) TO service_role;

COMMENT ON FUNCTION public.is_process_unique_worker_identity(text) IS 'Validates new v1 process-unique durable claimant IDs. Historical worker_id values are intentionally unchanged and remain readable.';
